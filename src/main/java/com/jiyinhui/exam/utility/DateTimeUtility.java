...
public class DateTimeUtility {
     ...
     * @throws ParseException
     */    
	public static int daysCeilBetween(Date smdate, Date bdate) {
		try {

			double between_days = dayTimeIntervals(smdate, bdate);
	...
	public static int daysFloorBetween(Date smdate, Date bdate) {
		try {
			double between_days = dayTimeIntervals(smdate, bdate);
	...
	private static double dayTimeIntervals(Date smdate, Date bdate) throws ParseException {
		Calendar cal = Calendar.getInstance();
		cal.setTime(smdate);
		long time1 = cal.getTimeInMillis();
		...
		double temp = time2 - time1;
		double dayTime = 1000 * 3600 * 24;
		double betweens = temp / dayTime;
		return betweens;
	}
     ...
     *          yearBase : false
     *          result   : 1
     *          如果yearBase为false只是年的相减
     *
     * @param start : 开始日期
     * @param end   : 结束日期
     * @param yearBase : true计算的是整年, false只是年的相减
     * @return
     */
    public static int yearBetween(Date start, Date end, boolean yearBase) {
        if (start.after(end)) {
            Date t = start;
            start = end;
            end = t;
